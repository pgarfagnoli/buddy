...
def _is_tool_result_envelope(msg: dict) -> bool:
    ...
    Claude Code writes every tool_use→tool_result round-trip as a `type: user`
    line whose `message.content` is a list of `tool_result` items. These
    interleave with assistant messages within a single turn and must not
...
def _sum_turn_usage(transcript_path: str) -> tuple[int, int, int]:
    ...
    file. Tool-result envelopes (`type: user` with content entirely made
    of `tool_result` blocks) are part of the current turn and are skipped
